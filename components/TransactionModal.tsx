...
const TransactionModal: React.FC<TransactionModalProps> = ({ isOpen, onClose, onSave, onAddClient, clients, initialData }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (initialData) {
      payload.id = initialData.id; // Preserve ID for updates
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden">
        <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100">
          <h3 className="text-lg font-bold text-slate-900">{initialData ? 'Editar Transação' : 'Nova Transação'}</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
             ...
             >
               {initialData ? 'Atualizar' : (mode === 'expense' ? 'Salvar Despesa' : 'Salvar Receita')}
             </button>
          </div>
        </form>
      </div>
